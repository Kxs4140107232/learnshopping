...
import com.apipay.demo.trade.DemoHbRunner;
import com.apipay.demo.trade.Main;
...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse pay(Integer userId, Long orderNo) {
        ...
        if (orderNo==null){
            return ServerResponse.createServerResponseByError("订单号不存在");
        }
        // 测试当面付2.0生成支付二维码
        Order order=orderMapper.findOrderByOrderNo(orderNo);
        if (order==null){
        ...
        return ServerResponse.createServerResponseByError("下单失败1");
    }
    ...
    public ServerResponse alipay_callback(Map<String, String> map) {
        ...
        if (order==null){
            return ServerResponse.createServerResponseByError("订单"+orderNo+"不是本商品订单");
        }
        if (order.getStatus()>=Const.OrderStatusEnum.ORDER_PAYED.getCode()){
            //防止支付宝重复回调
            return ServerResponse.createServerResponseByError("支付宝重复调用");
        }
        System.out.println(trade_status.equals(Const.TRADE_SUCCESS));
       /* System.out.println(DateUtils.strToDate(payment_time));*/
        ...
        if (trade_status.equals(Const.TRADE_SUCCESS)){
            ...
            int x = orderMapper.updateByPrimaryKey(order);
            System.out.println("修改结果"+x);
        }
        //保存支付信息
        ...
        int result=payInfoMapper.insert(payInfo);
            ...
            return ServerResponse.createServerResponseByError("信息保存失败");
    }
    ...
    public ServerResponse pay(Order order) throws IOException {
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl(" http://74y6dg.natappfree.cc/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("F:/ftpfile/qr-%s.png",
        ...
        return ServerResponse.createServerResponseByError("下单失败2");
    }
